...
require_text skills/test-driven-development/SKILL.md 'one proof per changed observable behavior.*not.*(function|helper)' 'TDD proofs follow behavior rather than implementation structure'
require_text skills/test-driven-development/SKILL.md 'RED/GREEN evidence.*test.*command.*expected reason.*(tree|content|HEAD)' 'TDD evidence binds commands and expected failure to content'
require_text skills/test-driven-development/SKILL.md 'characterization.*property.*integration' 'TDD accepts the right proof surface for existing behavior'
require_text skills/test-driven-development/SKILL.md '(tree|test|implementation).*changes.*rerun.*RED|rerun.*GREEN' 'TDD evidence is invalidated by relevant drift'
reject_text skills/test-driven-development/SKILL.md 'Every new function/method has a test' 'TDD does not require per-function tests'
require_text skills/writing-skills/SKILL.md 'For a new Skill.*no-Skill baseline' 'new skills retain a true no-guidance baseline'
require_text skills/writing-skills/SKILL.md 'For an existing Skill.*current behavior.*candidate.*important.*control' 'existing skills compare current and candidate behavior'
require_text skills/writing-skills/SKILL.md 'current Skill passes.*important.*control.*do not rewrite.*shared-contract' 'passing existing behavior is preserved absent a demonstrated contract change'
require_text skills/writing-skills/SKILL.md 'candidate.*must not regress.*control' 'skill edits preserve useful control behavior'
require_text skills/writing-skills/SKILL.md 'at least five fresh.*valid candidate samples' 'behavior-shaping skill edits use repeated fresh samples'
require_text skills/writing-skills/SKILL.md 'invalid or indeterminate.*report.*not.*count' 'skill evals expose unusable samples instead of hiding them'
require_text skills/writing-skills/SKILL.md 'integrated profile.*whole-set verification.*not.*after every.*Skill' 'coordinated skill batches deploy once after integrated verification'
reject_text skills/writing-skills/SKILL.md 'applies to NEW skills AND EDITS to existing skills' 'existing skill edits do not pretend the current skill is absent'
reject_text skills/writing-skills/SKILL.md 'After writing ANY skill, you MUST STOP and complete the deployment process' 'coordinated skill work is not interrupted by per-skill runtime deployment'
reject_text skills/writing-skills/SKILL.md 'deployment checklist below is MANDATORY for EACH skill' 'deployment cadence follows the integrated change rather than ritual batching'
require_text skills/systematic-debugging/SKILL.md 'evidence contract.*symptom.*reproduction.*observations.*hypothesis.*discriminating check.*root cause.*verification' 'debugging follows an evidence contract rather than ritual phases'
require_text skills/systematic-debugging/SKILL.md '[Ss]cale.*uncertainty' 'debugging effort scales with diagnostic uncertainty'
require_text skills/systematic-debugging/SKILL.md 'unambiguous.*minimal path' 'debugging permits direct proof for obvious causes'
reject_text skills/systematic-debugging/SKILL.md 'MUST complete each phase' 'debugging removes the universal four-phase gate'
reject_text skills/systematic-debugging/SKILL.md 'If.*(>=|≥) *3|3.*fixes.*STOP|three.*fixes.*STOP' 'debugging removes arbitrary attempt thresholds'
...
require_text skills/requesting-code-review/SKILL.md 'Review Binding.*full BASE.*HEAD.*working-tree.*requirements.*profile.*open questions.*verification' 'code review binds the exact content, requirements, questions, and evidence'
require_text skills/requesting-code-review/SKILL.md 'review returns.*recheck.*binding.*drift.*re-review' 'code review is invalidated by relevant content or evidence drift'
require_text skills/requesting-code-review/code-reviewer.md 'Reviewed binding' 'reviewer reports the exact binding it inspected'
require_text skills/receiving-code-review/SKILL.md 'per-finding decision.*accepted.*rejected.*needs clarification' 'review feedback is evaluated finding by finding'
require_text skills/receiving-code-review/SKILL.md 'unclear finding.*pauses.*finding.*depend.*continue.*independent' 'ambiguity blocks only dependent review work'
reject_text skills/receiving-code-review/SKILL.md 'IF any item is unclear|STOP - do not implement anything yet' 'one unclear finding does not globally block independent work'
...
require_text skills/dispatching-parallel-agents/SKILL.md '[Dd]etect.*runtime capacity.*live.*agents.*before every dispatch' 'parallel dispatch uses current runtime capacity'
reject_text skills/dispatching-parallel-agents/SKILL.md 'team limit is 4|at most 3 child agents' 'parallel dispatch does not hard-code one runtime topology'
...
require_text skills/dispatching-parallel-agents/SKILL.md 'Context Capsule.*capability.*goal.*inputs.*invariants.*owned paths.*expected artifacts.*evidence.*authorization.*fork_turns' 'parallel tasks carry a complete capability-scoped capsule'
require_text skills/dispatching-parallel-agents/SKILL.md 'child (summary|report).*(not|never).*proof.*controller.*(diff|evidence|verification)' 'controller verifies child results before integration'
...
require_text skills/subagent-driven-development/SKILL.md 'controller.*exact HEAD.*(run|observe).*deterministic verification' 'SDD controller directly proves task behavior on reviewed content'
require_text skills/subagent-driven-development/SKILL.md 'brief.*report.*review package.*content digest.*full commit SHA' 'SDD handoffs bind artifacts and commit identity'
require_text skills/subagent-driven-development/SKILL.md 'review returns.*binding.*changed.*regenerate.*re-review' 'SDD invalidates reviews after content drift'
...
require_text skills/finishing-a-development-branch/SKILL.md 'merge.*push.*publish_pr.*current.*bound.*verification evidence' 'integration and publication accept current content-bound verification evidence'
require_text skills/finishing-a-development-branch/SKILL.md 'binding.*unchanged.*reuse' 'unchanged verification bindings are reused instead of rerun ceremonially'
require_text skills/finishing-a-development-branch/SKILL.md 'binding drift.*rerun' 'verification reruns only after its binding drifts'
require_text skills/finishing-a-development-branch/SKILL.md 'git status --short.*empty.*immediately before' 'publication still checks a clean committed tree immediately before execution'
...
reject_text skills/finishing-a-development-branch/SKILL.md 'require fresh tests|without fresh passing tests' 'finishing does not invalidate still-bound verification evidence by timing alone'

require_text skills/subagent-driven-development/SKILL.md 'separate worktrees.*branches.*disjoint.*dispatching-parallel-agents' 'SDD permits isolated disjoint writer batches through the parallel-dispatch contract'
...
require_text docs/superpowers/specs/2026-07-14-superzhao-adaptive-control-plane-v2-design.md '^> \*\*SUPERSEDED' 'canceled control-plane spec is unmistakably historical and non-executable'
require_text docs/superpowers/specs/2026-07-14-superzhao-adaptive-control-plane-v2-design.md '2026-07-15.*scope override' 'historical spec names the user scope override'
require_text docs/superpowers/specs/2026-07-14-superzhao-adaptive-control-plane-v2-design.md 'timeout.*Task 2A.*14.*Skill' 'historical spec identifies only the retained work'
require_text docs/superpowers/plans/2026-07-14-superzhao-adaptive-control-plane-v2.md '^> \*\*SUPERSEDED' 'canceled implementation plan is unmistakably historical and non-executable'
require_text docs/superpowers/plans/2026-07-14-superzhao-adaptive-control-plane-v2.md 'Task 2B/2C.*must not be executed' 'historical plan cannot resume canceled Task 2B/2C work'
require_text docs/superpowers/approvals/2026-07-14-adaptive-control-plane-v2.json '"status": "superseded"' 'old approval record is marked superseded'
require_text docs/superpowers/approvals/2026-07-14-adaptive-control-plane-v2.json '"executable": false' 'old approval record is explicitly non-executable'
require_text docs/superpowers/approvals/2026-07-14-adaptive-control-plane-v2.json '"superseded_by": "2026-07-15 user scope override"' 'old approval points to the user override'
require_text docs/superpowers/approvals/2026-07-14-adaptive-control-plane-v2.json '"execution_condition": "Non-executable' 'old broad execution condition cannot authorize future work'

require_text docs/superpowers/evals/2026-07-15-superzhao-14-skills-behavior-evaluation.md 'behavior-evidence\.md' 'evaluation report links its auditable evidence artifact'
require_text docs/superpowers/evals/2026-07-15-superzhao-14-skills-behavior-evidence.md '^## Scenario Contract' 'behavior evidence records the scenario contract'
require_text docs/superpowers/evals/2026-07-15-superzhao-14-skills-behavior-evidence.md '^## Verdict Ledger' 'behavior evidence records per-sample verdicts and indeterminates'
require_text docs/superpowers/evals/2026-07-15-superzhao-14-skills-behavior-evidence.md '^## Raw Candidate Outputs' 'behavior evidence retains raw candidate outputs'
